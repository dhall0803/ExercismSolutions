...
awk '
    ...
    function homeDirectoryIsValid(homeDirectory) {
        return startsWith(homeDirectory, "/home") || startsWith(homeDirectory, "/root") 
    }

    BEGIN { FS = ":" }
    !homeDirectoryIsValid($6) { print }
' ./passwd
...
awk '
    ...
    BEGIN { FS = ":" }
    homeDirectoryIsValid($6) && $7 == "/bin/bash" { print }
' ./passwd
